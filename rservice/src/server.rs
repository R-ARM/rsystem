...
fn handle_client<T>(mut stream: UnixStream, methods: Arc<HashMap<String, Method<T>>>, state: Arc<Mutex<Box<T>>>) -> Option<()> {
    ...
    if let Some(function) = methods.get(&method) {
        let result = function(state, argv);
        reader.get_mut().write_all(result.as_str().as_bytes()).ok()?;
    }
